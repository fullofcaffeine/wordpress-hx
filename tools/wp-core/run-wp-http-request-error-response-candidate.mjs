...
const ISSUE = {
  id: "wordpresshx-3ct",
  external_ref: "WPHX-312.80",
  title: "WPHX-312.80 - Promote WP_Http request error response decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T03:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-error-response-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-error-response-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-80";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-80-wp-http-request-error-response-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-80-wp-http-request-error-response-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-80-wp-http-request-error-response-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestErrorResponseCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestErrorResponse\\HttpRequestErrorResponse_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request WP_Error response returns before nonblocking/filter decision"];
const CASES = [
  { id: "wp-http-request:exception-and-nonblocking", focus: "WP_Error response returns before nonblocking handling while non-error response continues" }
];
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_80_bootstrap_haxe' ) ) {
\tfunction wphx_312_80_bootstrap_haxe() {
...
\t\t$wphx_312_80_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_80_lib );
...
function transformCandidateRequestErrorResponse() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\tif ( is_wp_error( $response ) ) {
\t\t\treturn $response;
\t\t}
`;
  const replacement = `\t\tif ( ${HAXE_MODULE}::shouldReturnErrorResponse( is_wp_error( $response ) ) ) {
\t\t\treturn $response;
\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request error response branch");
...
\t\t$assertions['error_returned_before_nonblocking'] = 'http_request_failed' === $result['error']['code'];
\t\t$assertions['non_error_continues_to_nonblocking'] = array(
\t\t\t'headers' => array(),
\t\t\t'body' => '',
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-error-response-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request error response decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that a response identified as WP_Error after Requests dispatch and http_api_debug returns immediately before nonblocking and response-filter handling. PHP still owns public WP_Http::request ABI, Requests dispatch, exception conversion, WP_Error construction/internals, is_wp_error evaluation, http_api_debug timing/payload, nonblocking response shape, http_response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-error-response-candidate",
        "npm run wp:core:wphx-312-wp-http-request-error-response-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-80-wp-http-request-error-response-candidate"],
...
async function main() {
  ...
  transformCandidateRequestErrorResponse();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-error-response-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the post-debug WP_Error early-return branch decision in WP_Http::request is emitted by generated Haxe PHP. Requests dispatch, exception conversion, WP_Error construction, is_wp_error evaluation, debug action timing, nonblocking response shape, and response filtering remain PHP-owned."
    },
    fixture: {
      cases: CASES,
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted error response decision is a pure bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        ...
        detail:
          "Only the WP_Error early-return decision is Haxe-owned. Defaults, filters, preempt/error paths, streaming, header/cookie/SSL/proxy option assembly, Requests dispatch, exception conversion, WP_Error internals, is_wp_error evaluation, debug hooks, nonblocking response shape, and response filtering remain PHP-owned in this candidate."
      },
      {
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-80-wp-http-request-error-response-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request error response Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request error response Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request error-response candidate" }
    ],
