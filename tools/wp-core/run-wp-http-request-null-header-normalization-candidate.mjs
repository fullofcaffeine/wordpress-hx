...
const ISSUE = {
  id: "wordpresshx-rva",
  external_ref: "WPHX-312.87",
  title: "WPHX-312.87 - Promote WP_Http request null-header normalization decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-null-header-normalization-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-null-header-normalization-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-87";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-87-wp-http-request-null-header-normalization-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-87-wp-http-request-null-header-normalization-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-87-wp-http-request-null-header-normalization-candidate.v1.json";
...
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestNullHeaderNormalization\\HttpRequestNullHeaderNormalization_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request null headers normalize to empty array decision"];
const CASES = [
  { id: "wp-http-request:null-headers-normalize", focus: "explicit null headers normalize to an empty native array before hook construction and Requests dispatch" },
  { id: "wp-http-request:array-headers-preserve", focus: "existing array headers are preserved through the normalization branch" }
];
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_87_bootstrap_haxe' ) ) {
\tfunction wphx_312_87_bootstrap_haxe() {
...
\t\t$wphx_312_87_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_87_lib );
...
function transformCandidateRequestNullHeaderNormalization() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\tif ( is_null( $parsed_args['headers'] ) ) {
\t\t\t$parsed_args['headers'] = array();
\t\t}
`;
  const replacement = `\t\tif ( ${HAXE_MODULE}::shouldNormalizeHeaders( is_null( $parsed_args['headers'] ) ) ) {
\t\t\t$parsed_args['headers'] = array();
\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request null-header normalization decision branch");
...
switch ( $case ) {
\tcase 'wp-http-request:null-headers-normalize':
\t\t$response = $http->request( 'https://example.test/null-headers', array( 'headers' => null ) );
\t\t$call = $GLOBALS['wphx_requests_calls'][0] ?? array();
\t\t$result['response'] = wphx_summarize( $response );
\t\t$result['request_call'] = $call;
\t\t$assertions['requests_headers_empty_array'] = array() === $call['headers'];
\t\t$assertions['hook_request_headers_empty_array'] = array() === $call['options']['hook_request']['headers'];
\t\t$assertions['request_reached_dispatch'] = 'https://example.test/null-headers' === $call['url'];
\t\tbreak;

\tcase 'wp-http-request:array-headers-preserve':
\t\t$response = $http->request(
\t\t\t'https://example.test/array-headers',
\t\t\tarray(
\t\t\t\t'headers' => array( 'X-Array' => 'yes' ),
\t\t\t)
\t\t);
\t\t$call = $GLOBALS['wphx_requests_calls'][0] ?? array();
\t\t$result['response'] = wphx_summarize( $response );
\t\t$result['request_call'] = $call;
\t\t$assertions['requests_headers_preserved'] = array( 'X-Array' => 'yes' ) === $call['headers'];
\t\t$assertions['hook_request_headers_preserved'] = array( 'X-Array' => 'yes' ) === $call['options']['hook_request']['headers'];
\t\t$assertions['request_reached_dispatch'] = 'https://example.test/array-headers' === $call['url'];
\t\tbreak;
}
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-null-header-normalization-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request null-header normalization decision",
      area:
    ...
    verification: {
      ...
      receipt_refs: ["receipt:wphx-312-87-wp-http-request-null-header-normalization-candidate"],
...
async function main() {
  ...
  const manifest = {
    ...
    fixture: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted null-header-normalization decision is a pure bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-null-header-normalization-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the null-header normalization branch decision is Haxe-owned. Native parsed-args array mutation, header values, processHeaders semantics, Set-Cookie conversion, request defaults and filters, hook registration, cookies, redirects, SSL verification, max_bytes, data_format, Requests dispatch, response filtering, and actual transport behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      set_cookie_conversion_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-87-wp-http-request-null-header-normalization-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request null-header normalization Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request null-header normalization Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request null-header normalization candidate" }
    ],
