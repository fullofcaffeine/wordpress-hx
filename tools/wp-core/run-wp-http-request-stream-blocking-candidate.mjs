...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_72_bootstrap_haxe' ) ) {
\tfunction wphx_312_72_bootstrap_haxe() {
...
\t\t$wphx_312_72_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_72_lib );
...
function transformCandidateRequestStreamBlocking() {
	const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
	const original = `\t\t\t$parsed_args['blocking'] = true;
`;
	const replacement = `\t\t\tif ( ${HAXE_MODULE}::shouldForceBlockingForStream( (bool) $parsed_args['stream'] ) ) {
...
`;
  ...
  if (transformed === source) throw new Error("Unable to replace WP_Http::request stream blocking decision branch");
...
async function main() {
  ...
  transformCandidateRequestStreamBlocking();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request stream blocking assignment decision is emitted by generated Haxe PHP. The surrounding public method body, filename defaulting, writable-directory validation, and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
