...
const ISSUE = {
  id: "wordpresshx-ffd",
  external_ref: "WPHX-312.85",
  title: "WPHX-312.85 - Promote WP_Http request bad-protocol stripping decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T05:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-bad-protocol-stripping-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-bad-protocol-stripping-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-85";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-85-wp-http-request-bad-protocol-stripping-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-85-wp-http-request-bad-protocol-stripping-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-85-wp-http-request-bad-protocol-stripping-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestBadProtocolStrippingCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestBadProtocolStripping\\HttpRequestBadProtocolStripping_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request non-empty URL runs wp_kses_bad_protocol decision"];
const CASES = [
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_85_bootstrap_haxe' ) ) {
\tfunction wphx_312_85_bootstrap_haxe() {
...
\t\t$wphx_312_85_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_85_lib );
...
function transformCandidateRequestBadProtocolStripping() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\t\tif ( $url ) {
\t\t\t\t$url = wp_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
\t\t\t}
`;
  const replacement = `\t\t\tif ( ${HAXE_MODULE}::shouldStripBadProtocol( (bool) $url ) ) {
\t\t\t\t$url = wp_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
\t\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request bad-protocol stripping decision branch");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-bad-protocol-stripping-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request bad-protocol stripping decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that a non-empty current URL value runs wp_kses_bad_protocol() while the wp_kses_bad_protocol boundary exists. PHP still owns public request ABI, http_request_reject_unsafe_urls filtering, function_exists behavior, wp_http_validate_url semantics, URL mutation shape, bad-protocol stripping semantics, invalid-url WP_Error construction/debug timing, hook object mechanics, proxy option/auth handoff, pre_http_send_through_proxy overrides, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    bridge: {
      ...
      removal_gate:
        "Promote beyond this bounded compiler-emitted WP_Http::request branch adapter only after request proxy/safety, selected upstream HTTP PHPUnit, installed distribution, and live/recorded network/proxy parity gates before claiming full request, installed distribution, or whole-file WP_Http ownership."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-bad-protocol-stripping-candidate",
        "npm run wp:core:wphx-312-wp-http-request-bad-protocol-stripping-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-85-wp-http-request-bad-protocol-stripping-candidate"],
...
async function main() {
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-bad-protocol-stripping-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the non-empty current-URL branch decision before wp_kses_bad_protocol in WP_Http::request is emitted by generated Haxe PHP. Filtering, function availability, wp_http_validate_url semantics, URL mutation, bad-protocol stripping semantics, hook registration, proxy handoff, and Requests dispatch remain PHP-owned."
    },
    fixture: {
      cases: CASES,
    ...
    remaining_gaps: [
      {
        id: "full-request-bad-protocol-stripping-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the non-empty current-URL branch decision before bad-protocol stripping is Haxe-owned. Filtering, function_exists behavior, wp_http_validate_url semantics, URL mutation shape, wp_kses_bad_protocol semantics, invalid-url WP_Error construction/debug timing, hook registration mechanics, proxy handoff, Requests dispatch, and response/error handling remain PHP-owned in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      public_php_replacement_claimed: true,
      full_request_bad_protocol_stripping_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-85-wp-http-request-bad-protocol-stripping-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request bad-protocol stripping Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request bad-protocol stripping Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request bad-protocol stripping candidate" }
    ],
