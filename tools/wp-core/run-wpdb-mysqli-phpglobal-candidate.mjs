...
const ISSUE = {
  id: "wordpresshx-l76.9.20",
  external_ref: "WPHX-305.20",
  title: "Integrate @:phpGlobal mysqli externs into live wpdb candidate"
};
const HXML = "fixtures/wp-core/wpdb-mysqli-phpglobal-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-305-20";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const MYSQLI_EXECUTION_PHP = `${HAXE_OUT}/lib/wphx/wp/db/WpdbMysqliExecution.php`;
const OUT = "manifests/wp-core/wphx-305-20-wpdb-mysqli-phpglobal-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-305-20-wpdb-mysqli-phpglobal-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-305-20-wpdb-mysqli-phpglobal-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/WpdbMysqliPhpGlobalCandidateEntry.hx"
];
...
function writeAdminStub(root) {
  ...
  writeFileSync(
    ...
    `<?php
/**
 * WPHX-305.20 live database mysqli row traversal Haxe candidate stub.
 *
...
define( 'AUTH_SALT', 'wphx-305-20-auth-salt' );
...
function candidateMysqliIntegration() {
  ...
  return {
    id: "wphx-305-20-live-wpdb-phpglobal-mysqli-integration",
...
async function withDbRuntime(runtime, callback) {
  const name = `wordpresshx-wphx-305-20-${runtime.id}-${process.pid}`;
  let containerId = "";
  try {
...
function writeOrCheck(path, contents) {
  if (checkOnly) {
    ...
    if (current !== contents) {
      throw new Error(`${path} is stale; run npm run wp:core:wphx-305-mysqli-phpglobal-candidate`);
    }
    return;
  }
...
function ownershipManifest(manifestSha, upstreamDigest, runtimes) {
  return {
    ...
    manifest_id: "ownership:wp-core/wpdb-mysqli-phpglobal-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      kind: "module",
      name: "wpdb mysqli @:phpGlobal live database Haxe candidate",
      area: "wp-includes/wp-admin",
      public_contract:
        "WordPress-compatible maybe_create_table(), maybe_add_column(), dbDelta(), and wpdb::query/get_var/get_row/get_results remain callable with their public PHP class/function shape while selected schema, dbDelta, result-population, raw mysqli metadata, query/fetch call gates, fetched-row traversal decisions, and mysqli_query()/mysqli_fetch_object() execution delegate to typed Haxe. The WPHX_305_17_MysqliBoundary generated PHP class is removed from this candidate; generated Haxe PHP emits direct global \\mysqli_query($handle, $query) and \\mysqli_fetch_object($result) calls through @:phpGlobal externs."
    },
    ...
    smell_fixes: [
      {
        description:
          "Separated maybe_create_table()/maybe_add_column() branch decisions, bounded dbDelta classification, wpdb result-shape/null decisions, wpdb::query return/state branch decisions, _do_query timing/logging/execution/reconnect policy, selected-row last_result/num_rows population policy, fetched-row presence and assignment-slot policy, mysqli metadata read/fallback guards, mysqli query/fetch call gates, and mysqli query/fetch execution from handwritten/generated PHP boundaries into typed Haxe helpers. WPHX-305.20 removes the two-method WPHX_305_17_MysqliBoundary class from the live candidate and uses generated Haxe PHP direct global mysqli calls instead.",
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-305-mysqli-phpglobal-candidate",
...
if (!maybeCommand("docker", ["info", "--format", "{{.ServerVersion}}"])) {
  console.error(JSON.stringify({ status: "failed", error: "docker server unavailable; WPHX-305.20 requires live DB containers" }, null, 2));
  process.exit(1);
}
...
const manifest = {
  schema: "wphx.wp-core-wpdb-mysqli-phpglobal-candidate.v1",
  issue: ISSUE.external_ref,
  generated_at: RECORDED_AT,
  generator: "tools/wp-core/run-wpdb-mysqli-phpglobal-candidate.mjs",
  inputs: {
  ...
  fixture: {
    candidate_kind: "haxe_generated_wpdb_mysqli_phpglobal_shell",
    ...
    closes_gaps_from: [
      {
        ...
        resolution:
          "WPHX-305.20 extends the typed Haxe schema-helper/dbDelta candidate with bounded wpdb mysqli row traversal policy while retaining live MySQL/MariaDB execution through wpdb."
      },
      {
        manifest: LIVE_DB_CANDIDATE,
        gap: "wpdb-class-not-yet-haxe-owned",
        resolution:
          "WPHX-305.20 narrows a bounded wpdb query result traversal portion of this gap by routing mysqli_query()/mysqli_fetch_object() through typed Haxe @:phpGlobal execution helpers while preserving the WordPress-visible wpdb shell. Full wpdb replacement/drop-in behavior and storage ownership remain split to later WPHX-305 slices."
      },
      {
        manifest: DBDELTA_CANDIDATE,
        gap: "wpdb-class-not-yet-haxe-owned",
        resolution:
          "WPHX-305.20 keeps the dbDelta planning helper in the candidate while extending the wpdb class shell."
      },
      {
        manifest: QUERY_RESULT_CANDIDATE,
        gap: "wpdb-query-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.20 keeps the result-shape candidate in the live shell while routing selected-row traversal policy and mysqli query/fetch execution through typed Haxe helpers."
      },
      {
        manifest: QUERY_STATE_CANDIDATE,
        gap: "wpdb-native-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.20 keeps the query-state candidate in the live shell while adding typed Haxe decisions for fetched-row presence, loop break, last_result assignment slot, and row-count advancement."
      },
      {
        manifest: NATIVE_EXECUTION_CANDIDATE,
        gap: "wpdb-native-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.20 follows the native-execution candidate by keeping the WordPress-visible _do_query() shell while moving mysqli_query() execution into generated Haxe PHP through WpdbMysqliExecution::query()."
      },
      {
        manifest: RESULT_POPULATION_CANDIDATE,
        gap: "wpdb-raw-mysqli-resource-not-yet-haxe-owned",
        resolution:
          "WPHX-305.20 follows result-population ownership by keeping fetched-row loop break, last_result assignment-slot, and count advancement policy in typed Haxe around WpdbMysqliExecution::fetchObject()."
      },
      {
        manifest: RAW_RESOURCE_CANDIDATE,
        gap: "wpdb-raw-mysqli-query-result-fetch-not-yet-haxe-owned",
        resolution:
          "WPHX-305.20 narrows this gap with typed fetched-row traversal decisions and generated Haxe PHP direct global mysqli_query()/mysqli_fetch_object() calls."
      },
      {
        manifest: MYSQLI_BOUNDARY_CANDIDATE,
        gap: "wpdb-generated-mysqli-boundary-not-yet-replaced-by-direct-haxe-lowering",
        resolution:
          "WPHX-305.20 replaces the generated two-method mysqli boundary in the live candidate with WpdbMysqliExecution generated from typed Haxe @:phpGlobal externs."
      },
      {
        manifest: MYSQLI_ROW_TRAVERSAL_CANDIDATE,
        gap: "wpdb-generated-mysqli-boundary-not-yet-replaced-by-direct-haxe-lowering",
        resolution:
          "WPHX-305.20 preserves the WPHX-305.17 row traversal behavior while removing WPHX_305_17_MysqliBoundary from the candidate shell."
      },
      {
        manifest: MYSQLI_GLOBAL_LOWERING_PROOF,
        gap: "proof-not-yet-integrated-into-live-wpdb-candidate",
        resolution:
          "WPHX-305.20 integrates the WPHX-305.19 @:phpGlobal lowering proof into the live wpdb candidate and reruns the MySQL/MariaDB oracle comparison."
      }
    ],
    follows: ["WPHX-305.19", "WPHX-305.17", "WPHX-305.16", "WPHX-305.15", "WPHX-305.14", "WPHX-305.13", "WPHX-305.12", "WPHX-305.11", "WPHX-305.10", "WPHX-305.08", "WPHX-305.07", "WPHX-305.06"]
  },
  toolchain: {
  ...
  remaining_gaps: [
    {
      id: "native-row-object-value-materialization-not-yet-typed",
      owner: "future WPHX-305 row materialization workset",
      detail: "This candidate routes mysqli_fetch_object() through typed Haxe-generated PHP and keeps fetched-row presence/assignment/count policy in typed Haxe, but the returned stdClass row value is still consumed by the public PHP wpdb shell. A later slice should model row objects and field access more precisely without breaking WordPress result-shape ABI."
    },
    {
...
const receipt = {
  ...
  id: "receipt:wphx-305-20-wpdb-mysqli-phpglobal-candidate",
  issue: ISSUE,
  recorded_at: RECORDED_AT,
  artifacts: [
    {
      path: OUT,
      role: "wpdb mysqli @:phpGlobal live MySQL/MariaDB Haxe candidate manifest"
    },
    {
      path: OWNERSHIP,
    ...
    {
      path: "tools/wp-core/run-wpdb-mysqli-phpglobal-candidate.mjs",
      role: "wpdb mysqli @:phpGlobal live DB candidate generator and check-mode validator"
    },
    {
