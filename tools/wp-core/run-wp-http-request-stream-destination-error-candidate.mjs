...
const ISSUE = {
  id: "wordpresshx-loq",
  external_ref: "WPHX-312.82",
  title: "WPHX-312.82 - Promote WP_Http request stream destination error decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-stream-destination-error-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-stream-destination-error-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-82";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-82-wp-http-request-stream-destination-error-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-82-wp-http-request-stream-destination-error-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-82-wp-http-request-stream-destination-error-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestStreamDestinationErrorCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestStreamDestinationError\\HttpRequestStreamDestinationError_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request unwritable stream destination returns WP_Error decision"];
const CASES = [
  { id: "wp-http-request:streaming", focus: "valid stream continues to Requests and unwritable stream destination returns WP_Error" }
];
...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_82_bootstrap_haxe' ) ) {
\tfunction wphx_312_82_bootstrap_haxe() {
...
\t\t$wphx_312_82_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_82_lib );
...
function transformCandidateRequestStreamDestinationError() {
	const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
	const original = `\t\t\tif ( ! wp_is_writable( dirname( $parsed_args['filename'] ) ) ) {
...
\t\t\t}
`;
	const replacement = `\t\t\tif ( ${HAXE_MODULE}::shouldReturnStreamDestinationError( wp_is_writable( dirname( $parsed_args['filename'] ) ) ) ) {
\t\t\t\t$response = new WP_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
\t\t\t\t/** This action is documented in wp-includes/class-wp-http.php */
\t\t\t\tdo_action( 'http_api_debug', $response, 'response', 'WpOrg\\Requests\\Requests', $parsed_args, $url );
\t\t\t\treturn $response;
...
`;
  ...
  if (transformed === source) throw new Error("Unable to replace WP_Http::request stream destination error decision branch");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-stream-destination-error-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request stream destination error decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that a PHP-owned stream destination writability check failure returns the streaming destination WP_Error before Requests dispatch. PHP still owns public WP_Http::request ABI, request defaults and filters, stream filename defaulting, dirname/wp_is_writable evaluation, WP_Error construction, translation, http_api_debug timing/payload, forced blocking behavior, header/cookie/redirect/SSL/max_bytes/data_format handoff, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-stream-destination-error-candidate",
        "npm run wp:core:wphx-312-wp-http-request-stream-destination-error-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-82-wp-http-request-stream-destination-error-candidate"],
...
async function main() {
  ...
  transformCandidateRequestStreamDestinationError();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-stream-destination-error-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request stream destination error branch decision over PHP-owned wp_is_writable evaluation is emitted by generated Haxe PHP. The surrounding public method body, filename defaulting, WP_Error/debug handling, and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted stream-destination-error decision is a pure bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-stream-destination-error-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the unwritable stream destination branch decision is Haxe-owned. Parsed argument defaults/filters, filename defaulting, dirname/wp_is_writable evaluation, WP_Error construction, translation, http_api_debug timing/payload, forced blocking behavior, hook registration, headers, cookies, redirects, SSL verification, max_bytes, data_format, Requests dispatch, response filtering, and actual streaming transfer behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-82-wp-http-request-stream-destination-error-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request stream destination error Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request stream destination error Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request stream destination error candidate" }
    ],
