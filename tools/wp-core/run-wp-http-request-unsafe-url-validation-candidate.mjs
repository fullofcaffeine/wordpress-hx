...
const ISSUE = {
  id: "wordpresshx-zea",
  external_ref: "WPHX-312.84",
  title: "WPHX-312.84 - Promote WP_Http request unsafe URL validation decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T05:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-unsafe-url-validation-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-unsafe-url-validation-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-84";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-84-wp-http-request-unsafe-url-validation-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-84-wp-http-request-unsafe-url-validation-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-84-wp-http-request-unsafe-url-validation-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestUnsafeUrlValidationCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestUnsafeUrlValidation\\HttpRequestUnsafeUrlValidation_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request reject_unsafe_urls runs wp_http_validate_url decision"];
const CASES = [
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_84_bootstrap_haxe' ) ) {
\tfunction wphx_312_84_bootstrap_haxe() {
...
\t\t$wphx_312_84_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_84_lib );
...
function transformCandidateRequestUnsafeUrlValidation() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\t\tif ( $parsed_args['reject_unsafe_urls'] ) {
\t\t\t\t$url = wp_http_validate_url( $url );
\t\t\t}
`;
  const replacement = `\t\t\tif ( ${HAXE_MODULE}::shouldValidateUnsafeUrl( function_exists( 'wp_kses_bad_protocol' ), (bool) $parsed_args['reject_unsafe_urls'] ) ) {
\t\t\t\t$url = wp_http_validate_url( $url );
\t\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request unsafe URL validation decision branch");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-unsafe-url-validation-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request unsafe URL validation decision",
      area:
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-unsafe-url-validation-candidate",
        "npm run wp:core:wphx-312-wp-http-request-unsafe-url-validation-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-84-wp-http-request-unsafe-url-validation-candidate"],
...
async function main() {
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-unsafe-url-validation-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the reject_unsafe_urls URL-validation branch decision in WP_Http::request is emitted by generated Haxe PHP. Filtering, function availability, wp_http_validate_url semantics, URL mutation, bad-protocol stripping, hook registration, proxy handoff, and Requests dispatch remain PHP-owned."
    },
    fixture: {
      cases: CASES,
    ...
    remaining_gaps: [
      {
        id: "full-request-unsafe-url-validation-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the reject_unsafe_urls URL-validation branch decision is Haxe-owned. Filtering, function_exists behavior, wp_http_validate_url semantics, URL mutation shape, bad-protocol stripping, invalid-url WP_Error construction/debug timing, hook registration mechanics, proxy handoff, Requests dispatch, and response/error handling remain PHP-owned in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      public_php_replacement_claimed: true,
      full_request_unsafe_url_validation_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-84-wp-http-request-unsafe-url-validation-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request unsafe URL validation Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request unsafe URL validation Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request unsafe URL validation candidate" }
    ],
