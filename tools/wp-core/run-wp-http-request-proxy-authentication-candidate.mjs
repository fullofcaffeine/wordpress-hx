...
const ISSUE = {
  id: "wordpresshx-pv8",
  external_ref: "WPHX-312.79",
  title: "WPHX-312.79 - Promote WP_Http request proxy authentication decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T05:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-proxy-authentication-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-proxy-authentication-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-79";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-79-wp-http-request-proxy-authentication-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-79-wp-http-request-proxy-authentication-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-79-wp-http-request-proxy-authentication-candidate.v1.json";
...
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestProxyAuthentication\\HttpRequestProxyAuthentication_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request proxy authentication decision"];
const CASES = [
  ...
  { id: "wp-http-request-proxy:pre-filter-overrides", focus: "proxy option handoff without authentication keeps proxy credentials unset" }
];
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_79_bootstrap_haxe' ) ) {
\tfunction wphx_312_79_bootstrap_haxe() {
...
\t\t$wphx_312_79_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_79_lib );
...
function transformCandidateRequestProxyAuthentication() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\t\tif ( $proxy->use_authentication() ) {
\t\t\t\t$options['proxy']->use_authentication = true;
\t\t\t\t$options['proxy']->user               = $proxy->username();
\t\t\t\t$options['proxy']->pass               = $proxy->password();
\t\t\t}
`;
  const replacement = `\t\t\tif ( ${HAXE_MODULE}::shouldUseProxyAuthentication( (bool) $proxy->use_authentication() ) ) {
\t\t\t\t$options['proxy']->use_authentication = true;
\t\t\t\t$options['proxy']->user               = $proxy->username();
\t\t\t\t$options['proxy']->pass               = $proxy->password();
\t\t\t}
`;
  ...
  if (transformed === source) throw new Error("Unable to replace WP_Http::request proxy authentication condition");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-proxy-authentication-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request proxy authentication decision",
      area:
    ...
    verification: {
      ...
      receipt_refs: ["receipt:wphx-312-79-wp-http-request-proxy-authentication-candidate"],
...
async function main() {
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_HTTP_Proxy::use_authentication branch decision in WP_Http::request is emitted by generated Haxe PHP. Proxy enable/send-through policy, pre_http_send_through_proxy filtering, proxy object construction, username/password values, Requests dispatch, and response handling remain PHP-owned."
    },
    fixture: {
      cases: CASES,
    ...
    remaining_gaps: [
      {
        id: "full-request-proxy-authentication-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the use_authentication branch decision is Haxe-owned. WP_HTTP_Proxy enablement, host/bypass matching, pre_http_send_through_proxy filtering, proxy address construction, username/password values, native Requests proxy object behavior, Requests dispatch, response filtering, and actual proxy negotiation remain PHP-owned or unclaimed in this candidate."
      },
      {
        id: "live-requests-network-and-proxy-io-not-executed",
        owner: ISSUE.external_ref,
        detail:
          "The fixture records WP_Http's Requests and proxy authentication handoff through a stubbed Requests::request boundary. It does not perform live network I/O, DNS, TLS, proxy negotiation, redirect following, or transport execution."
      },
      {
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-79-wp-http-request-proxy-authentication-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request proxy authentication Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request proxy authentication Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request proxy authentication candidate" }
    ],
