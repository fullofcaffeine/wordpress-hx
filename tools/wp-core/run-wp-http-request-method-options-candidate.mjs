...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_68_bootstrap_haxe' ) ) {
\tfunction wphx_312_68_bootstrap_haxe() {
...
\t\t$wphx_312_68_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_68_lib );
...
function transformCandidateRequestMethodOptions() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\tif ( 'HEAD' !== $type && 'GET' !== $type ) {
\t\t\t$options['data_format'] = 'body';
\t\t}
`;
  const replacement = `\t\tif ( ${HAXE_MODULE}::shouldUseBodyDataFormat( (string) $type ) ) {
\t\t\t$options['data_format'] = 'body';
\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request data_format decision branch");
...
async function main() {
  ...
  transformCandidateRequestMethodOptions();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request method-derived data_format='body' decision is emitted by generated Haxe PHP. The surrounding public method body and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
