...
const ISSUE = {
  id: "wordpresshx-b9j",
  external_ref: "WPHX-312.90",
  title: "WPHX-312.90 - Promote WP_Http request mbstring reset decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T03:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-mbstring-reset-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-mbstring-reset-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-90";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-90-wp-http-request-mbstring-reset-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-90-wp-http-request-mbstring-reset-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-90-wp-http-request-mbstring-reset-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestMbstringResetCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestMbstringReset\\HttpRequestMbstringReset_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request resets mbstring encoding after Requests dispatch/catch"];
const CASES = [
  { id: "wp-http-request:exception-and-nonblocking", focus: "mbstring reset runs after exception and successful nonblocking Requests dispatches" }
];
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_90_bootstrap_haxe' ) ) {
\tfunction wphx_312_90_bootstrap_haxe() {
...
\t\t$wphx_312_90_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_90_lib );
...
function transformCandidateRequestMbstringReset() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\treset_mbstring_encoding();
`;
  const replacement = `\t\tif ( ${HAXE_MODULE}::shouldResetMbstringEncodingAfterDispatch() ) {
\t\t\treset_mbstring_encoding();
\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request mbstring reset branch");
...
\t\t$assertions['reset_after_exception_and_success'] = 2 === count( array_filter( $GLOBALS['wphx_actions'], static function ( $action ) { return 'reset_mbstring_encoding' === $action['hook']; } ) );
\t\t$assertions['binary_safe_before_each_dispatch'] = 2 === count( array_filter( $GLOBALS['wphx_actions'], static function ( $action ) { return 'mbstring_binary_safe_encoding' === $action['hook']; } ) );
\t\t$assertions['nonblocking_shape_preserved'] = array(
\t\t\t'headers' => array(),
\t\t\t'body' => '',
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-mbstring-reset-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request mbstring reset decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that reset_mbstring_encoding() runs after the Requests dispatch/catch boundary before http_api_debug, error, nonblocking, and response-filter handling. PHP still owns public WP_Http::request ABI, mbstring encoding semantics, Requests dispatch, exception conversion, response conversion, debug/filter timing, error and nonblocking exits, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-mbstring-reset-candidate",
        "npm run wp:core:wphx-312-wp-http-request-mbstring-reset-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-90-wp-http-request-mbstring-reset-candidate"],
...
async function main() {
  ...
  transformCandidateRequestMbstringReset();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-mbstring-reset-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request post-dispatch mbstring reset decision is emitted by generated Haxe PHP. mbstring encoding semantics, Requests dispatch, exception conversion, response conversion, debug action timing, error/nonblocking exits, and response filtering remain PHP-owned."
    },
    fixture: {
      cases: CASES,
      ...
      native_boundary_policy: {
        ...
        native_false_and_null_required: false,
        escape_hatch_owner: null,
        detail: "The promoted mbstring reset decision is a pure no-argument bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        ...
        detail:
          "Only the decision to run reset_mbstring_encoding() after the Requests dispatch/catch boundary is Haxe-owned. Defaults, filters, preempt/error paths, streaming, header/cookie/SSL/proxy option assembly, Requests dispatch, exception conversion, response conversion, mbstring internals, debug hooks, nonblocking response shape, and response filtering remain PHP-owned in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      mbstring_reset_decision_claimed: true,
      mbstring_encoding_semantics_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-90-wp-http-request-mbstring-reset-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request mbstring reset Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request mbstring reset Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request mbstring-reset candidate" }
    ],
