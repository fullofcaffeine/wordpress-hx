...
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpCookieHeaderAssembly\\HttpCookieHeaderAssembly_Fields_";
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_63_bootstrap_haxe' ) ) {
\tfunction wphx_312_63_bootstrap_haxe() {
...
\t\t$wphx_312_63_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_63_lib );
...
function transformCandidateCookieHeaderAssembly() {
...
\t\t\t$cookies_header = ${HAXE_MODULE}::appendCookieHeader( $cookies_header, $cookie->getHeaderValue() );
\t\t}
...
function ownershipManifest(manifestSha) {
  return {
    ...
    owned_paths: [RUNNER, HXML, "src/wphx/wp/http/HttpCookieHeaderAssembly.hx", "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpCookieHeaderAssemblyCandidateEntry.hx", OUT, OWNERSHIP, RECEIPT],
...
async function main() {
  ...
  transformCandidateCookieHeaderAssembly();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      public_shell_policy: {
        ...
        shell_body_ownership:
          "temporary candidate shell preserves by-reference request array mutation, scalar-to-WP_Http_Cookie upgrading, WP_Http_Cookie::getHeaderValue(), and filter timing while delegating Cookie header append/separator assembly to generated Haxe PHP",
        native_boundaries: [
    ...
    remaining_gaps: [
      ...
      },
      {
        ...
        owner: ISSUE.external_ref,
        detail:
          "processResponse, processHeaders, chunkTransferDecode, protected parse_url, and broader live header/transport behavior remain separate PHP boundaries or previously scoped candidates."
      },
      {
