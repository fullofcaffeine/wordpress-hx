...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_73_bootstrap_haxe' ) ) {
\tfunction wphx_312_73_bootstrap_haxe() {
...
\t\t$wphx_312_73_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_73_lib );
...
function transformCandidateRequestStreamFilenameOptions() {
	const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
	const original = `\t\tif ( $parsed_args['stream'] ) {
\t\t\t$options['filename'] = $parsed_args['filename'];
\t\t}
`;
	const replacement = `\t\tif ( ${HAXE_MODULE}::shouldSetStreamFilenameOption( (bool) $parsed_args['stream'] ) ) {
\t\t\t$options['filename'] = $parsed_args['filename'];
\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request stream filename options decision branch");
...
async function main() {
  ...
  transformCandidateRequestStreamFilenameOptions();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request stream filename option assignment decision is emitted by generated Haxe PHP. The surrounding public method body, filename defaulting, writable-directory validation, and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
