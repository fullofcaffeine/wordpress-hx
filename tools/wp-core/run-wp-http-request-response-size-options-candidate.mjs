...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_71_bootstrap_haxe' ) ) {
\tfunction wphx_312_71_bootstrap_haxe() {
...
\t\t$wphx_312_71_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_71_lib );
...
function transformCandidateRequestResponseSizeOptions() {
	const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
	const original = `\t\tif ( isset( $parsed_args['limit_response_size'] ) ) {
\t\t\t$options['max_bytes'] = $parsed_args['limit_response_size'];
\t\t}
`;
	const replacement = `\t\tif ( ${HAXE_MODULE}::shouldSetMaxBytes( $parsed_args['limit_response_size'] ) ) {
\t\t\t$options['max_bytes'] = $parsed_args['limit_response_size'];
\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request response-size option decision branch");
...
async function main() {
  ...
  transformCandidateRequestResponseSizeOptions();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request response-size option branch decision is emitted by generated Haxe PHP. The surrounding public method body, parsed argument value, and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
