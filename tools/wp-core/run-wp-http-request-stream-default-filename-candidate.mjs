...
const ISSUE = {
  id: "wordpresshx-liz",
  external_ref: "WPHX-312.88",
  title: "WPHX-312.88 - Promote WP_Http request stream default-filename decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-stream-default-filename-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-stream-default-filename-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-88";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-88-wp-http-request-stream-default-filename-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-88-wp-http-request-stream-default-filename-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-88-wp-http-request-stream-default-filename-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestStreamDefaultFilenameCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestStreamDefaultFilename\\HttpRequestStreamDefaultFilename_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request streaming empty filename uses temp basename default decision"];
const CASES = [
  { id: "wp-http-request:streaming", focus: "streaming with empty filename gets temp basename default while explicit filename is preserved" }
];
...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_88_bootstrap_haxe' ) ) {
\tfunction wphx_312_88_bootstrap_haxe() {
...
\t\t$wphx_312_88_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_88_lib );
...
function transformCandidateRequestStreamDefaultFilename() {
	const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
	const original = `\t\t\tif ( empty( $parsed_args['filename'] ) ) {
\t\t\t\t$parsed_args['filename'] = get_temp_dir() . basename( $url );
\t\t\t}
`;
	const replacement = `\t\t\tif ( ${HAXE_MODULE}::shouldUseDefaultStreamFilename( (bool) $parsed_args['stream'], ! empty( $parsed_args['filename'] ) ) ) {
\t\t\t\t$parsed_args['filename'] = get_temp_dir() . basename( $url );
\t\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request stream default-filename decision branch");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-stream-default-filename-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request stream default-filename decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request branch decision that streaming requests with an empty parsed filename should assign the PHP-owned get_temp_dir() . basename(url) default filename. PHP still owns public WP_Http::request ABI, request defaults and filters, temp-dir lookup, basename/path semantics, native parsed-args mutation, writable-directory validation, forced blocking behavior, header/cookie/redirect/SSL/max_bytes/data_format handoff, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-stream-default-filename-candidate",
        "npm run wp:core:wphx-312-wp-http-request-stream-default-filename-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-88-wp-http-request-stream-default-filename-candidate"],
...
async function main() {
  ...
  transformCandidateRequestStreamDefaultFilename();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-stream-default-filename-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request stream default-filename branch decision is emitted by generated Haxe PHP. The surrounding public method body, get_temp_dir/basename evaluation, native parsed-args mutation, writable-directory validation, and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted stream-default-filename decision is a pure bool/bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-stream-default-filename-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the empty-filename defaulting branch decision is Haxe-owned. Parsed argument defaults/filters, get_temp_dir and basename/path semantics, native parsed-args mutation, writable-directory validation, forced blocking behavior, Requests filename option handoff, hook registration, headers, cookies, redirects, SSL verification, max_bytes, data_format, Requests dispatch, response filtering, and actual streaming transfer behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      stream_default_filename_decision_claimed: true,
      temp_dir_or_basename_semantics_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-88-wp-http-request-stream-default-filename-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request stream default-filename Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request stream default-filename Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request stream default-filename candidate" }
    ],
