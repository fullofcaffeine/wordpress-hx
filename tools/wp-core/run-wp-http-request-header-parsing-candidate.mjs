...
const ISSUE = {
  id: "wordpresshx-zrf",
  external_ref: "WPHX-312.77",
  title: "WPHX-312.77 - Promote WP_Http request header parsing decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-header-parsing-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-header-parsing-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-77";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-77-wp-http-request-header-parsing-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-77-wp-http-request-header-parsing-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-77-wp-http-request-header-parsing-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestHeaderParsingCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestHeaderParsing\\HttpRequestHeaderParsing_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request non-array headers route through processHeaders decision"];
const CASES = [
  { id: "wp-http-request:head-defaults", focus: "default array headers bypass request header parsing" },
  { id: "wp-http-request:headers-cookies-ssl-post", focus: "string headers route through processHeaders before Requests dispatch" }
];
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_77_bootstrap_haxe' ) ) {
\tfunction wphx_312_77_bootstrap_haxe() {
...
\t\t$wphx_312_77_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_77_lib );
...
function transformCandidateRequestHeaderParsing() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\tif ( ! is_array( $parsed_args['headers'] ) ) {
\t\t\t$processed_headers      = self::processHeaders( $parsed_args['headers'] );
\t\t\t$parsed_args['headers'] = $processed_headers['headers'];
\t\t}
`;
  const replacement = `\t\tif ( ${HAXE_MODULE}::shouldParseHeaders( is_array( $parsed_args['headers'] ) ) ) {
\t\t\t$processed_headers      = self::processHeaders( $parsed_args['headers'] );
\t\t\t$parsed_args['headers'] = $processed_headers['headers'];
\t\t}
`;
  ...
  if (transformed === source) throw new Error("Unable to replace WP_Http::request header parsing decision branch");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-header-parsing-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request header parsing decision",
      area:
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-header-parsing-candidate",
        "npm run wp:core:wphx-312-wp-http-request-header-parsing-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-77-wp-http-request-header-parsing-candidate"],
...
async function main() {
  ...
  transformCandidateRequestHeaderParsing();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-header-parsing-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request non-array headers branch decision is emitted by generated Haxe PHP. The surrounding public method body, null normalization, native header array values, processHeaders implementation, option assembly, Requests dispatch, and response filtering remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted header-parsing decision is a pure bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-header-parsing-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the non-array headers branch decision is Haxe-owned. Null-to-empty-array normalization, native header arrays, processHeaders semantics, Set-Cookie conversion, request defaults and filters, hook registration, cookies, redirects, SSL verification, max_bytes, data_format, Requests dispatch, response filtering, and actual transport behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      null_headers_normalization_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-77-wp-http-request-header-parsing-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request header parsing Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request header parsing Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request header parsing candidate" }
    ],
