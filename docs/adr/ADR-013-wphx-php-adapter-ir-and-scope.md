...
The v0 implementation in `src/wphx/compiler/php/WphxPhpCompiler.hx` covers the currently proven shell shapes: original-path files, guarded global functions, classes/interfaces, bounded direct file-scope script adapters, methods, properties, constants, Haxe runtime bootstrap markers, protected methods, by-reference parameters, grouped original-path public class adapters, and emission-manifest declarations. `WPHX-COMP-PHP-CORE-IR-NATIVE-ARRAYS` and `WPHX-COMP-PHP.07` add the first reusable PHP-core method-body nodes for conditionals, `for`/`foreach` loops, `break`, `continue`, `return`, `throw`, native array reads/writes/appends, array casts, bool/int/string casts, long array literals, object construction, local variables, assignments, object property reads, class constants, and calls. `WPHX-COMP-PHP-GROUP-WP-HTTP-HELPERS` proves that one generated original-path `WP_Http` shell can group `processResponse`, `chunkTransferDecode`, protected `parse_url`, `buildCookieHeader( &$r )`, `processHeaders( $headers, $url = '' )`, `is_ip_address`, `browser_redirect_compatibility`, and `validate_redirects` without unsupported constructs while preserving the bounded public/protected ABI and helper behavior. `WPHX-COMP-PHP-INCLUDE-SIDE-EFFECTS` adds the first bounded original-path direct script adapter for include timing, include returns, function-scope locals, repeated include/include_once behavior, and output buffering. `WPHX-COMP-PHP-PROFILE-ADAPTER-REGISTRY` moves the WordPress-specific `wp-http-*` method body construction into `src/wphx/compiler/php/WphxPhpWordPressAdapters.hx`, while the compiler core continues to own generic PHP IR, feature recording, and printing. These nodes are reusable compiler core where possible; the WordPress profile only chooses when an original-path public boundary needs them.
