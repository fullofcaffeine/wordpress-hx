...
Do not assume stock Haxe PHP can emit the public WordPress distribution files modern Core requires. Use stock Haxe PHP first for private implementation output and Haxe stdlib/runtime behavior; use WPHX PHP for bounded original-path public adapters where file identity, declaration timing, references, native arrays, warnings, stack traces, reflection, or include behavior are the contract. WPHX PHP is the active staged custom compiler lane, so design Haxe ABI metadata and Adapter IR as future-backend inputs, not as disposable shell hints. If the stock Haxe PHP target cannot emit an idiomatic private implementation shape, or if WPHX PHP starts becoming a general backend, reduce the problem to a generic compiler-pressure fixture and either add reusable WPHX PHP core nodes, improve stock Haxe PHP, or promote WPHX PHP toward a broader Reflaxe/custom PHP backend with evidence and an ADR. Do not let convenience PHP strings in runners become the architecture.
