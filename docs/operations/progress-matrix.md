...
Source checkpoint: `WPHX-307.03 post CRUD/status adapter-contract candidate`
Tracking issue: `WPHX-000.02`

## Super Progress

Current super progress toward "WordPress completely in Haxe and functional": **38.8%**.
...
| WordPress PHP runtime Haxe ownership | 40 | 20 | 8.0 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, user/auth and posts/query Haxe adapter-contract candidates, auth-domain oracle fixtures, and WPHX-307 posts/query plus post CRUD/status adapter-contract evidence, but much of Core remains unported |
