...
Source checkpoint: `WPHX-310.01 themes/theme JSON/template hierarchy surface inventory`
...
| WordPress PHP runtime Haxe ownership | 40 | 30.0 | 12.0 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, bounded WPHX-306 users/auth, WPHX-307 posts/query, WPHX-308 taxonomy/comment, WPHX-309 routing/template domain closures, and the WPHX-310 themes/theme JSON/template hierarchy surface inventory, but much of Core remains unported |
...
| `WPHX-310` Themes/theme JSON/template hierarchy | active | `WPHX-310.01` adds a bounded surface inventory for 127 PHP runtime source files, 152 PHP distribution artifacts, 1,786 ABI entries, and 141 upstream tests across theme JSON/global styles, theme model/supports, customizer, nav menus, widgets/sidebars, block theme templates, theme compat, admin theme screens, duotone, script modules, and related template helpers. This is inventory evidence only: it does not claim Haxe runtime ownership, generated public PHP replacement, installed WordPress behavior, or upstream parity. Next gates are typed Haxe adapter-contract candidates, classic and block theme fixtures, theme.json/global styles, theme-support/customizer/widget/nav-menu coverage, installed front-end/admin gates, and selected upstream ratchets. |
