...
Source checkpoint: `WPHX-308.09 installed-distribution taxonomy/comment gate`
Tracking issue: `WPHX-000.02`

## Super Progress

Current super progress toward "WordPress completely in Haxe and functional": **42.2%**.
...
| WordPress PHP runtime Haxe ownership | 40 | 28.5 | 11.4 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, bounded WPHX-306 users/auth, WPHX-307 posts/query, and WPHX-308 taxonomy/comment domain closures, plus WPHX-309 routing/template surface inventory, first typed routing/template adapter-contract evidence, and first routing/link oracle-state fixture evidence, but much of Core remains unported |
...
| `WPHX-300` | WordPress PHP core | open | 8/8 original direct children closed; WPHX-306, WPHX-307, and WPHX-308 are closed; WPHX-309 has surface inventory, first adapter-contract candidate, and first routing/link oracle-state fixture; WPHX-310 through WPHX-320 remain seeded follow-up domains | Bootstrap, hooks, error/formatting/escaping/KSES, options/cache, wpdb, REST, multisite, first-party PHP manifest/split closure, users/auth adapter-contract/domain-closure evidence, bounded posts/query domain closure, bounded taxonomy/comment domain closure, rewrite/routing/canonical/template surface inventory, first typed routing/template adapter-contract candidate, and first routing/link oracle-state fixture have receipts; broad Core implementation remains open through remaining seeded follow-up domains | Continue WPHX-309 with canonical/template-loader oracle fixtures, front-end HTTP observations, and selected upstream ratchets |
...
| `WPHX-309` Rewrite/routing/canonical URLs/templates | active | `WPHX-309.01` adds a surface inventory receipt for 15 classified source files, 13 distribution artifacts, 254 ABI entries, 3 classes, and 78 upstream test paths across request entrypoints, `WP`, `WP_Rewrite`, rewrite helpers, canonical redirects, link generation, template-loader, and sitemap-index handoffs. `WPHX-309.04` adds the first typed Haxe routing/template adapter-contract candidate for route classification, request-plan intent, canonical redirect intent, link-template routing, template-loader hierarchy intent, and routing/template hook intent. `WPHX-309.06` adds the first deterministic oracle-source-mirror routing/link fixture for selected `WP_Rewrite` tag/rule/permastruct state, `home_url`, `user_trailingslashit`, and `WP::parse_request` matched, extra-query-var, and 404 observations, wired into deterministic CI. This remains bounded evidence: it does not claim public PHP replacement, Haxe-owned runtime semantics for routing/link behavior, full canonical redirect parity, template-loader include behavior, front-end HTTP parity, or installed-distribution parity. Next gates are canonical/template-loader oracle fixtures, front-end HTTP installed-distribution observations, selected upstream rewrite/canonical/link/template PHPUnit ratchets, and eventually generated public PHP adapter replacement. |
