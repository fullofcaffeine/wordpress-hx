...
Source checkpoint: `WPHX-306.07 installed auth distribution gate and WPHX-306 domain closure`
...
| `WPHX-300` | WordPress PHP core | open | 8/8 original direct children closed; WPHX-306 is closed; WPHX-307 through WPHX-320 remain seeded follow-up domains | Bootstrap, hooks, error/formatting/escaping/KSES, options/cache, wpdb, REST, multisite, first-party PHP manifest/split closure, and users/auth adapter-contract/domain-closure evidence have receipts; broad Core implementation remains open through remaining seeded follow-up domains | Start WPHX-307, then continue WPHX-308 through WPHX-320 by source-domain priority |
